...
**Current development state:** the native overlay, structured-lyrics pipeline, offline Japanese annotation, local audio-transcription engine, model installer, monitor recovery, startup registration, package archives and compact settings controls are implemented. Windows SMTC detects media metadata; LRCLIB and NetEase provide synchronized lyrics; IPADIC and JMdict supply readings and concise meanings. The application automatically prefers accepted synchronized lyrics and otherwise starts local Japanese Whisper transcription after a short transition delay. Structured lyrics stop capture immediately, while ASR text follows the same IPADIC/JMdict annotation path. This is not yet called MVP-complete because the full physical player, game, mixed-DPI, sleep/wake and output-device acceptance matrix remains open.
