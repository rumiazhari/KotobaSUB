    ...
    public static int Run(string[] args)
    {
        ...
        app.Dispatcher.BeginInvoke(DispatcherPriority.ApplicationIdle, new Action(() =>
        {
            try
            {
                ...
                Check(panel.Children.OfType<Slider>().Count() == 10, "settings exposes weight, spacing and layer opacity sliders");
                ...
                panel.Children.OfType<Slider>().Skip(4).First().Value = .6;
                Check(edited?.JapaneseOpacity == .6, "settings Japanese opacity applies live");
                panel.Children.OfType<Slider>().Skip(5).First().Value = .7;
