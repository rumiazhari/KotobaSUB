...
## Internet lyrics only mode — 2026-09-16

This opt-in mode disables local audio capture and ASR evidence completely. Provider lyrics remain authoritative and continue to resolve through the existing LRCLIB and NetEase sources. As a result, automatic audio verification and learned alignment updates are paused; a previously learned profile can still be applied to provider timestamps. No audio is captured or processed while the setting is on.
