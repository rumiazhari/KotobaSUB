...
Milestone 2 implements attributed cleanup/matching adaptations in `src/KotobaSUB.Core/Adapted/FlyingLyrics/MetadataMatching.cs`; THIRD_PARTY_NOTICES lists the source/revision and differences. GPLv3 remains in LICENSE. Network romanization, analytics, browser storage, UI and pseudo-sync were not ported. Native/local-kana/explicit-alias comparison is implemented; dictionary-backed kanji aliases remain part of the Japanese-analysis work.
