...
Missing lyric recovery (2026-09-16): local logs confirm provider matches followed by a WPF cross-thread exception. Fix session continuation affinity, restore ASR display without SMTC metadata, and search past untimed top candidates within a twelve-fetch budget. Validate with synchronization-context regression, provider fixtures, native suite and actual metadata-free routing smoke.
Completed and validated the missing-lyric fixes with full native tests and controlled WPF/browser rendering. Live loopback recognition remains unverified after fixture timeout.
Artist benchmarks (2026-09-16): test public Kotoha and Nemo Suzumi originals/covers, record provider IDs and failures without audio/lyrics persistence. Fix explicit video metadata formatting and transient HTTP failures; retain performer, recording-version and duration gates. Browser control currently fails to initialize, so streaming acceptance cannot yet be claimed.

## Internet lyrics only setting — 2026-09-16

Add a persistent Settings-window switch that keeps network lyric resolution enabled but suppresses WASAPI capture, Whisper fallback and ASR-based lyric verification. Reject queued ASR callbacks after a change and do not restart an unavailable ASR session on Retry while it remains enabled. Validate persistence, router behavior and the native preview before packaging.
