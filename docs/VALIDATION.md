...
The native Settings window now persists **Internet lyrics only (disable local ASR)**. The deterministic suite verifies settings roundtrip plus routing behavior: no structured timeline produces neither ASR output nor a capture request, while an existing structured timeline still renders. Full native verification remains required after the implementation change.
